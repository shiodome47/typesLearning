...
Compact 9件・Effect 10件を1つのアプリで扱い、一覧で切り替えられる。
...
|---|---|---|---|---|
| 目的 | **読んで判断できる**こと＋**白紙から組み立てられる**こと | **手が覚えている**こと（AIが使えないときの保険） | **公開してよいか判断できる**こと | **握りつぶしを見抜ける**こと＋**Promise では書けないものを書ける**こと |
| 構文の暗記 | 不要 | 中核だけは必要 | 不要（構文は小さい） | 不要（核だけ） |
| 診断の比率 | 5/48 | 8/31（「動くが間違っている」が多いため） | 2/9 | 3/10 |
| 件数 | 37 + スクラッチ編 11 | 16 + SvelteKit 編 9 + ガードレール編 6 | 9 | 前半 5 + 後半 5 |
...
①〜⑤は「型に何が出ているか」の話でした。後半は**書けるものが増える**話です。

| 回 | 扱うもの | `Promise` だとどうなるか |
|---|---|---|
| ⑥ | 中断（`fork` / `Fiber.interrupt`） | `Promise` にキャンセルは無い。捨てても走り続ける |
| ⑦ | 後始末（`acquireRelease` / `Scope`） | `try/finally` は中断では走らず、書き忘れも型で止まらない |
| ⑧ | 並行（`Effect.all` / `race`） | `Promise.all` は同時本数を指定できず、失敗しても残りが走る |
| ⑨ | 境界（`Schema.decodeUnknown`） | `as` は実行時に何もしない。`JSON.parse` は `any` |
| ⑩ | **診断：止まらない処理と閉じない資源** | ⑥〜⑨が1つずつ抜けた状態。全部コンパイルは通る |

要点は3つです。

**⑥ 止める権利が型に出る。** `Promise<T>` を受け取った人にできるのは待つか無視するかだけですが、
`Fiber<T, E>` を受け取った人は止められます。`AbortController` は `fetch` にだけ効く別建ての仕組みで、
型には現れません。

**⑦ 閉じ忘れがコンパイルエラーになる。** `Effect.acquireRelease` は `R` に `Scope` を足します。
`Effect.runPromise` は `R = never` しか受け取らないので、`Effect.scoped` で範囲を決めるまで実行できません。
リソースリークが、本番のメモリ監視ではなく書いている最中に止まります。

**⑨ 形を「値」として書く。** TypeScript の型は実行時に消えるので、外から来た値を検査できません。
`as` しか残らなかったのはそのためです。`Schema.Struct` は実行時に存在する値なので検査でき、
その失敗が `Effect` のエラー型に入るため、②と同じ検査（握りつぶすと通らない）にかかります。

⑩はこの4つを1つずつ抜いたコードを読ませます。**4つとも型エラーは出ません。**
見るのは「戻り値が `void` か」「`R` に `Scope` があるか」「`concurrency` が書いてあるか」
「エラー型に検証の失敗があるか」の4点です。
...
│   └── effect/                     # Effect 編 10件（独立言語。採点は TS の型診断）
├── scripts/
