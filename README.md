...
TypeScript 41件（Effect 編 4件を含む）・Svelte 31件（SvelteKit 編 9件・ガードレール編 6件を含む）・
Compact 8件を1つのアプリで扱い、一覧で切り替えられる。
...
| 診断の比率 | 5/41 | 8/31（「動くが間違っている」が多いため） | 1/8 |
| 件数 | 37 + Effect 編 4 | 16 + SvelteKit 編 9 + ガードレール編 6 | 8 |
...
│   ├── compact/                    # Compact 編 8件（連続チュートリアル）
│   └── effect/                     # Effect 編 4件（失敗・依存を型に出す）
├── scripts/
