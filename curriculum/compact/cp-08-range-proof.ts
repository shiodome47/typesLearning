...
export const cpLesson08: Lesson = {
  ...
  why: {
    ...
    insight:
      ...
      "**★そして、その「回路の中で検証する」に落とし穴があります。**\n\n" +
      "まず、Compact に「署名を検証する」組み込み関数はありません。" +
      "`ecMulGenerator` / `ecMul` / `ecAdd` / `hashToCurve` を組み合わせて自分で書きます（数行です）。\n\n" +
      "問題はその先です。\n" +
      "**回路の中で検証できるのは、埋め込み曲線の上に載った署名だけ**です。\n\n" +
      "実世界の PKI は、たいてい別の曲線で署名しています（P-256 など。" +
      "マイナンバーカードの署名もこれです）。" +
      "**それは回路の中では検証できません。** 実装の都合ではなく、曲線が違うということです。\n\n" +
      "だから現実にはこうなります。\n\n" +
      "```\n" +
      "実世界の署名 → 回路の外で検証し、埋め込み曲線の鍵で署名し直す → 回路で検証\n" +
      "```\n\n" +
      "真ん中が入った瞬間、**回路が保証しているのは「その鍵で署名されたこと」までになります。**\n" +
      "「元の署名を正しく確認したうえで署名し直したか」は、回路の外の信頼です。" +
      "その鍵が漏れれば、すべての証明書を偽造できます。\n\n" +
      "**欠陥ではありません。曲線が違うことの必然です。**\n" +
      "ただ書いておかないと、**回路がすべてを保証しているように見えます**。\n" +
      "次の⑦で扱うのと同じ形の落とし穴で、こちらのほうが大きい版です。",
  },
  explanation:
