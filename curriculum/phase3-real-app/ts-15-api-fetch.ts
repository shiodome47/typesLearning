...
export const lesson15: Lesson = {
  ...
  why: {
    problem:
      "サーバーから利用者の情報を取ってきて、メールアドレスを画面に小文字で表示する。よくある処理です。\n\n" +
      "仕様書には「email は文字列」と書いてあったので、そのとおりに型を書きました。" +
      "手元では完璧に動きます。テストも通ります。リリースします。\n\n" +
      "数日後、一部の利用者だけ画面が真っ白になる、と連絡が来ます。" +
      "調べると、メールアドレスを登録していない人がいて、サーバーは `email` に `null` を返していました。" +
      "仕様書には書かれていなかったのです。\n\n" +
      "ここが肝心なところです。TypeScript は**一度も警告を出しませんでした**。" +
      "`as User` と書いた瞬間、あなたは「これは User です」と宣言したことになり、TypeScript はそれを信じます。" +
      "実際に届いたデータを見に行ったりはしません。\n\n" +
      "型は「自分が書いたコードの中」を守る仕組みです。" +
      "外から来たデータ（API、ユーザー入力、localStorage）に対しては、何の力もありません。",
    insight:
      "`as` は検査ではありません。「文句を言わないでくれ」と型チェッカーに頼む言葉です。\n\n" +
      "代わりに使うのが `unknown` です。`unknown` は「まだ何か分からない値」という意味で、" +
      "**中身を確かめるまで何にも使えません**。`user.email` と書こうとするとエラーになります。" +
      "これは不便なのではなく、確認を強制してくれているのです。\n\n" +
      "確認するのが型ガード（`value is User`）です。" +
      "`typeof v.email === \"string\"` のように実際の値を1つずつ調べて、全部そろっていたら true を返します。" +
      "true が返ったあとだけ、TypeScript はその値を User として扱ってくれます。\n\n" +
      "順番が逆になっているのが要点です。" +
      "`as` は「User だと決めてから使う」。型ガードは「User だと確かめてから使う」。" +
      "確かめているので、`null` が来たらその場で気づけます。数日後の問い合わせではなく。\n\n" +
      "実務では zod や valibot というライブラリでこの確認を短く書きますが、やっていることはまったく同じです。",
  },
  explanation:
